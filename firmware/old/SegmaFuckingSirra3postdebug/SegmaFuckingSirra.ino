...
// --- ULTRASUONI ---
// Il task misura ogni INTERVALLO_ULTRASUONI ms (7 secondi).
// Usa pulseIn() bloccante, che funziona correttamente anche con WiFi attivo.
// Per non disturbare il loop principale il task gira su Core 0
// (il WiFi stack usa Core 0 ma non interferisce con pulseIn hardware).
const unsigned long INTERVALLO_ULTRASUONI = 7000;  // ms tra una misura e la successiva
const int           CAMPIONI_ULTRASUONI   = 3;      // media su N campioni
const unsigned long PULSIN_TIMEOUT_US     = 30000;  // 30ms timeout per pulseIn
const unsigned long PAUSA_TRA_PING_MS     = 50;     // pausa tra campioni consecutivi
...
// Timestamp ultimi aggiornamenti
...
//
// Usa pulseIn() bloccante — l'unica soluzione affidabile su ESP32
// con WiFi attivo. Il task gira su Core 0 con priorità 1 e si mette
// in sleep per INTERVALLO_ULTRASUONI tra una misura e l'altra,
// quindi non disturba il loop principale su Core 1.
// ============================================================
...
void taskUltrasuoni(void* param) {
    ...
    for (;;) {
        // Aspetta prima di misurare (non misurare continuamente)
        ...
        for (int i = 0; i < CAMPIONI_ULTRASUONI; i++) {
            // Assicura che trig sia basso prima di triggerare
            digitalWrite(trigPin, LOW);
            delayMicroseconds(4);

            // Impulso trigger 10µs
            digitalWrite(trigPin, HIGH);
            delayMicroseconds(10);
            digitalWrite(trigPin, LOW);

            // pulseIn è bloccante ma hardware: non dipende dallo scheduler
            ...
            // Pausa tra campioni per evitare interferenze eco precedente
...
void setup() {
    ...
    Serial.printf("[INIT] echoPin GPIO%d a riposo: %s %s\n",
                  ...
                  echoStato ? "!! ATTENZIONE: dovrebbe essere LOW !!" : "(OK)");
    ...
    servoA.attach(servoAPin); servoA.write(90);
    ...
    // Il loop principale (sensori, firebase, LCD) gira su Core 1.
    // Il task dorme la maggior parte del tempo (vTaskDelay 7s),
    // quindi non crea contesa. Quando si sveglia usa pulseIn()
    // che è hardware e non viene interrotto dallo scheduler.
...
void loop() {
    ...
    // Controllo periodico connessione WiFi
    ...
    // Lettura sensori (temp, umidità, suolo, luce)
    ...
    // Aggiornamento LCD + riepilogo debug
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        if (!deserializeJson(doc, payload)) {
            ...
            if (doc.containsKey("roofOpen")) {
                ...
                servoA.write(roofOpen ? 180 : 0);
                servoB.write(roofOpen ? 0 : 180);
                DBG_ATT("Tetto %s\n", roofOpen ? "APERTO" : "CHIUSO");
            }
